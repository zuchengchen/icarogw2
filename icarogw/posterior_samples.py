...
class posterior_samples_catalog(object):
    ...
    def build_parallel_posterior(self, nparallel=None):
        ...
        nsamps = np.array(
            [
                self.posterior_samples_dict[key].nsamples
                for key in self.posterior_samples_dict.keys()
            ]
        )
        ...
        print(
            "Using {:d} samples from each {:d} posteriors".format(
                self.nparallel, self.n_ev
            )
        )
        ...
        self.posterior_parallel = {
            key: xp.empty(
                [self.n_ev, self.nparallel],
                dtype=self.posterior_samples_dict[llev[0]].posterior_data[key].dtype,
            )
            for key in self.posterior_samples_dict[llev[0]].posterior_data.keys()
        }
        ...
        for i, event in enumerate(list(self.posterior_samples_dict.keys())):
            ...
            for key in self.posterior_parallel.keys():
                self.posterior_parallel[key][i, :] = self.posterior_samples_dict[
                    event
                ].posterior_data[key][rand_perm[: self.nparallel]]
            self.posterior_samples_dict[
                event
            ].numpyfy()  # Big data is forced to be on CPU

    def cupyfy(self):
        """Converts all the posterior samples to cupy"""
        self.posterior_parallel = {
            key: np2cp(self.posterior_parallel[key]) for key in self.posterior_parallel
        }

    def numpyfy(self):
        """Converts all the posterior samples to numpy"""
        self.posterior_parallel = {
            key: cp2np(self.posterior_parallel[key]) for key in self.posterior_parallel
        }
    ...
    def update_weights(self, rate_wrapper):
        ...
        self.log_weights = rate_wrapper.log_rate_PE(
            self.posterior_parallel["prior"],
        ...
        self.sum_weights = (
            xp.exp(sx.special.logsumexp(self.log_weights, axis=1)) / self.nparallel
        )
        self.sum_weights_squared = xp.exp(
            sx.special.logsumexp(2 * self.log_weights, axis=1)
        ) / xp.power(self.nparallel, 2.0)
    ...
    def get_effective_number_of_PE(self):
        ...
        Neff_vect = xp.power(self.sum_weights, 2.0) / self.sum_weights_squared
        Neff_vect[xp.isnan(Neff_vect)] = 0.0
        return Neff_vect
    ...
    def reweight_PE(self, rate_wrapper, Nsamp, replace=True):
        ...
        return {
            key: self.posterior_dict[key].reweight_PE(
                rate_wrapper, Nsamp, replace=replace
            )
            for key in name_ev
        }
...
class posterior_samples(object):
    def __init__(self, posterior_dict, prior):
        ...
        """
        self.posterior_data = {
            key: posterior_dict[key] for key in posterior_dict.keys()
        }
    ...
    def pixelize(self, nside):
        ...
        nside: integer
            Nside for healpy
        """
        self.posterior_data["sky_indices"] = radec2indeces(
            self.posterior_data["right_ascension"],
            self.posterior_data["declination"],
            nside,
        )
        self.nside = nside
    ...
    def cupyfy(self):
        """Converts all the posterior samples to cupy"""
        self.posterior_data = {
            key: np2cp(self.posterior_data[key]) for key in self.posterior_data
        }

    def numpyfy(self):
        """Converts all the posterior samples to numpy"""
        self.posterior_data = {
            key: cp2np(self.posterior_data[key]) for key in self.posterior_data
        }
    ...
    def add_counterpart(self, z_EM, ra, dec):
        ...
        print(
            "There are {:d} samples in the EM counterpart direction".format(len(select))
        )
        self.posterior_data = {
            key: self.posterior_data[key] for key in self.posterior_data.keys()
        }
    ...
    def reweight_PE(self, rate_wrapper, Nsamp, replace=True):
        ...
        logw = rate_wrapper.log_rate_PE(
            **{key: self.posterior_data[key] for key in self.posterior_data.keys()}
        )
        xp = get_module_array(logw)
        prob = xp.exp(logw)
        prob /= prob.sum()
        idx = xp.random.choice(
            len(self.posterior_data["prior"]), replace=replace, p=prob
        )
        return {
            key: self.posterior_data[key][idx]
            for key in list(self.posterior_data.keys())
        }
