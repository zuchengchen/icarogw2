...
def user_normal(x, mu, sigma):
    ...
    return xp.power(2 * xp.pi * (sigma**2), -0.5) * xp.exp(
        -0.5 * xp.power((x - mu) / sigma, 2.0)
    )


# LVK Reviewed
def EM_likelihood_prior_differential_volume(
    z, zobs, sigmaz, cosmology, Numsigma=1.0, ptype="uniform"
):
    """
    ...
    if ptype == "uniform":
        ...
        zvalmax = np.minimum(zobs + 5.0 * sigmaz, cosmology.zmax)
        if zvalmax <= zvalmin:
        ...
        prior_eval = (
            4
            * xp.pi
            * cosmology.dVc_by_dzdOmega_at_z(z)
            * ((z >= (zobs - Numsigma * sigmaz)) & (z <= (zobs + Numsigma * sigmaz)))
            / (cosmology.z2Vc(zvalmax) - cosmology.z2Vc(zvalmin))
        )
    elif ptype == "gaussian":
        ...
        normfact = xp.trapz(
            cosmology.dVc_by_dzdOmega_at_z(zproxy) * user_normal(zproxy, zobs, sigmaz),
            zproxy,
        )
...
class galaxy_catalog(object):
    ...
    def create_hdf5(self, filename, cat_data, band, nside):
        ...
        for key in list(cat_data.keys()):
            ...
            cat_data = {
                subkey: cat_data[subkey][tokeep] for subkey in list(cat_data.keys())
            }
    ...
    def load_hdf5(self, filename, cosmo_ref=None, epsilon=None):
        ...
        try:
            if self.hdf5pointer["catalog/mthr_map"].attrs["mthr_percentile"] == "empty":
                ...
                if is_there_cupy:
                    self.mthr_map_gpu = np2cp(
                        self.hdf5pointer["catalog/mthr_map/mthr_sky"][:]
                    )
        ...
        try:
            if cosmo_ref is None:
                raise ValueError(
                    "You need to provide a cosmology if you want to load the interpolant"
                )
            ...
            self.pixel_grid_cpu = np.arange(
                0, self.hdf5pointer["catalog"].attrs["npixels"], 1
            ).astype(int)
    ...
    def calculate_mthr(self, mthr_percentile=50, nside_mthr=None):
        ...
        skypixmthr = radec2indeces(
            self.hdf5pointer["catalog/ra"][:],
            self.hdf5pointer["catalog/dec"][:],
            nside_mthr,
        )
        ...
        if mthr_percentile != "empty":
            ...
            mthgroup.attrs["sky_checkpoint"] = (
                self.hdf5pointer["catalog"].attrs["npixels"] - 1
            )
            self.mthr_map_cpu = "empty"
            if is_there_cupy:
                self.mthr_map_gpu = "empty"
    ...
    def calc_dN_by_dzdOmega_interpolant(
        self,
        cosmo_ref,
        epsilon,
        Nintegration=10,
        ...
        try:
            interpogroup = self.hdf5pointer["catalog"].create_group(
                "dNgal_dzdOm_interpolant"
            )
        ...
        idx_in_range = np.where(
            ((cat_data["z"][:] - Numsigma * cat_data["sigmaz"][:]) <= zcut)
            & ((cat_data["z"][:] + Numsigma * cat_data["sigmaz"][:]) >= 1e-6)
        )[0]
        if len(idx_in_range) == 0:
            raise ValueError(
                "There are no galaxies in the redshift range 1e-6 - {:f}".format(zmax)
            )
        ...
        for i in tqdm(idx_in_range, desc="Looping on galaxies to find width"):
            ...
            if zmax >= cosmo_ref.zmax:
                print(zmin, zmax)
                raise ValueError(
                    "The maximum redshift for interpolation is too high w.r.t the cosmology class"
                )
        ...
        for i in tqdm(skyloop, desc="Calculating interpolant"):
            ...
            if len(gal_index) == 0:
                ...
                tos[:] = np.nan
                interpogroup.create_dataset(
                    "vals_pixel_{:d}".format(i), data=tos, dtype=np.float16
                )
                del tos
                continue
            ...
            for gal in gal_index:
                ...
                Mv = m2M(
                    cat_data["m"][idx_in_range[gal]],
                    cosmo_ref.z2dl(z_grid),
                    self.calc_kcorr(z_grid),
                )
                interpo += (
                    absM_rate.evaluate(self.sch_fun, Mv)
                    * EM_likelihood_prior_differential_volume(
                        z_grid,
                        cat_data["z"][idx_in_range[gal]],
                        cat_data["sigmaz"][idx_in_range[gal]],
                        cosmo_ref,
                        Numsigma=Numsigma,
                        ptype=ptype,
                    )
                    / self.hdf5pointer["catalog"].attrs["dOmega_sterad"]
                )

            interpo[interpo == 0.0] = np.nan
            interpo = np.float16(np.log(interpo))
            interpogroup.create_dataset(
                "vals_pixel_{:d}".format(i), data=interpo, dtype=np.float16
            )

        self.hdf5pointer.close()

    def effective_galaxy_number_interpolant(
        self, z, skypos, cosmology, dl=None, average=False
    ):
        """
        ...
        if isinstance(self.mthr_map_cpu, str):
            return xp.zeros(len(z)).reshape(originshape), (
                self.sch_fun.background_effective_galaxy_density(
                    -xp.inf * xp.ones_like(z)
                )
                * cosmology.dVc_by_dzdOmega_at_z(z)
            ).reshape(originshape)
        ...
        if average:
            gcpart = xp.interp(z, z_grid, dNgal_dzdOm_sky_mean, left=0.0, right=0.0)
        else:
            gcpart = sx.interpolate.interpn(
                (z_grid, pixel_grid),
                dNgal_dzdOm_vals,
                xp.column_stack([z, skypos]),
                bounds_error=False,
                fill_value=0.0,
                method="linear",
            )  # If a posterior samples fall outside, then you return 0

        bgpart = self.sch_fun.background_effective_galaxy_density(
            Mthr_array
        ) * cosmology.dVc_by_dzdOmega_at_z(z)
    ...
    def check_differential_effective_galaxies(self, z, radec_indices_list, cosmology):
        ...
        gcp, bgp, inco = (
            np.zeros([len(z), len(radec_indices_list)]),
            np.zeros([len(z), len(radec_indices_list)]),
            np.zeros([len(z), len(radec_indices_list)]),
        )

        for i, skypos in enumerate(radec_indices_list):
            gcp[:, i], bgp[:, i] = self.effective_galaxy_number_interpolant(
                z, skypos * np.ones_like(z).astype(int), cosmology
            )
            Mthr_array = self.calc_Mthr(
                z, np.ones_like(z, dtype=int) * skypos, cosmology
            )
            Mthr_array[z > self.z_grid_cpu[-1]] = -np.inf
            inco[:, i] = self.sch_fun.background_effective_galaxy_density(
                Mthr_array
            ) / self.sch_fun.background_effective_galaxy_density(
                -np.ones_like(Mthr_array) * np.inf
            )

        fig, ax = plt.subplots(2, 1, sharex=True)

        theo = self.sch_fun.background_effective_galaxy_density(
            -np.inf * np.ones_like(z)
        ) * cosmology.dVc_by_dzdOmega_at_z(z)

        ax[0].fill_between(
            z,
            np.percentile(gcp, 5, axis=1),
            np.percentile(gcp, 95, axis=1),
            color="limegreen",
            alpha=0.2,
        )
        ax[0].plot(
            z, np.median(gcp, axis=1), label="Catalog part", color="limegreen", lw=2
        )

        ax[0].plot(
            z, np.median(bgp, axis=1), label="Background part", color="slateblue", lw=2
        )

        ax[0].fill_between(
            z,
            np.percentile(bgp + gcp, 5, axis=1),
            np.percentile(bgp + gcp, 95, axis=1),
            color="tomato",
            alpha=0.2,
        )
        ...
        ax[1].fill_between(
            z,
            np.percentile(1 - inco, 5, axis=1),
            np.percentile(1 - inco, 95, axis=1),
            color="dodgerblue",
            alpha=0.5,
        )
        ax[1].plot(
            z,
            np.median(1 - inco, axis=1),
            label="Completeness",
            color="dodgerblue",
            lw=1,
        )
