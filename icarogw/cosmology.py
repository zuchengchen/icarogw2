...
class base_cosmology(object):
    ...
    def _checkz(self, z):
        ...
        if (smin < 1e-6) | (smax > self.zmax):
            raise ValueError(
                "Redshift provided not in range 1e-6<z<{:.2f}, zmin = {:f}, zmax = {:f}".format(
                    self.zmax, smin, smax
                )
            )
    ...
    def _checkdl(self, dl):
        ...
        dlmingrid, dlmaxgrid = xp.power(10.0, log10_dl_at_z.min()), xp.power(
            10.0, log10_dl_at_z.max()
        )

        if (dlmin < dlmingrid) | (dlmax > dlmaxgrid):
            raise ValueError(
                "Luminosity provided not in range {:f}<dl<{:f} Mpc, dlmin = {:f} Mpc, dlmax = {:f} Mpc".format(
                    dlmingrid, dlmaxgrid, dlmin, dlmax
                )
            )
...
class astropycosmology(base_cosmology):
    def build_cosmology(self, astropy_cosmo):
        ...
        self.little_h = astropy_cosmo.H(0.0).value / 100.0

        self.log10_dVc_dzdOmega_cpu = (
            np.log10(astropy_cosmo.differential_comoving_volume(self.z_cpu).value) - 9.0
        )  # Conversion from Mpc to Gpc
        self.log10_Vc_cpu = (
            np.log10(astropy_cosmo.comoving_volume(self.z_cpu).value) - 9.0
        )  # Conversion to Gpc
        self.log10_dl_at_z_cpu = np.log10(
            astropy_cosmo.luminosity_distance(self.z_cpu).value
        )
        self.log10_ddl_by_dz_cpu = np.log10(
            (np.power(10.0, self.log10_dl_at_z_cpu) / (1.0 + self.z_cpu))
            + COST_C * (1.0 + self.z_cpu) / astropy_cosmo.H(self.z_cpu).value
        )
...
class extraD_astropycosmology(astropycosmology):
    def build_cosmology(self, astropy_cosmo, D, n, Rc):
        ...
        dlem = np.power(10.0, self.log10_dl_at_z_cpu)
        dlbydz_em = np.power(10.0, self.log10_ddl_by_dz_cpu)

        self.log10_dl_at_z_cpu = np.log10(
            dlem
            * np.power(
                1 + np.power(dlem / ((1.0 + self.z_cpu) * Rc), n), (D - 4.0) / (2 * n)
            )
        )
        Afa = 1.0 + np.power(dlem / ((1.0 + self.z_cpu) * Rc), n)
        expo = (D - 4.0) / (2.0 * n)
        ...
        self.log10_ddl_by_dz_cpu = np.log10(
            np.abs(
                ...
                * (
                    ...
                    * (expo * n / Afa)
                    * (
                        dlbydz_em / np.power(1.0 + self.z_cpu, n)
                        - dlem / np.power(1.0 + self.z_cpu, n + 1.0)
                    )
                )
            )
        )
...
class cM_astropycosmology(astropycosmology):
    def build_cosmology(self, astropy_cosmo, cM):
        ...
        ZforI = np.append(0.0, ZforI)
        Zhalfbin = (ZforI[:-1:] + ZforI[1::]) * 0.5
        Integrandhalfin = 1.0 / (
            (1 + Zhalfbin) * np.power(astropy_cosmo.efunc(Zhalfbin), 2.0)
        )
        Integrand = 1.0 / (
            (1 + self.z_cpu) * np.power(astropy_cosmo.efunc(self.z_cpu), 2.0)
        )
        ...
        # We put the absolute value for the Jacobian (because this is needed for probabilities)
        self.log10_ddl_by_dz_cpu = np.log10(
            np.abs(exp_factor * dlbydz_em + 0.5 * dlem * cM * exp_factor * Integrand)
        )
...
class Xi0_astropycosmology(astropycosmology):
    def build_cosmology(self, astropy_cosmo, Xi0, n):
        ...
        dlbydz_em = np.power(10.0, self.log10_ddl_by_dz_cpu)
        self.log10_dl_at_z_cpu = np.log10(
            dlem * (Xi0 + (1.0 - Xi0) * np.power(1.0 + self.z_cpu, -n))
        )
        # We put the absolute value for the Jacobian (because this is needed for probabilities)
        self.log10_ddl_by_dz_cpu = np.log10(
            np.abs(
                dlbydz_em * (Xi0 + (1.0 - Xi0) * np.power(1 + self.z_cpu, -n))
                - dlem * (1.0 - Xi0) * n * np.power(1 + self.z_cpu, -n - 1)
            )
        )
...
class alphalog_astropycosmology(astropycosmology):
    def build_cosmology(self, astropy_cosmo, alphalog_1, alphalog_2, alphalog_3):
        ...
        self.log10_dl_at_z_cpu = np.log10(
            ...
            * (
                1.0
                + alphalog_1 * np.log1p(self.z_cpu)
                + alphalog_2 * (np.log1p(self.z_cpu) ** 2.0)
                + alphalog_3 * (np.log1p(self.z_cpu) ** 3.0)
            )
        )
        part1 = dlbydz_em * (
            1.0
        ...
        part2 = (dlem / (1 + self.z_cpu)) * (
            alphalog_1
            + alphalog_2 * 2.0 * np.log1p(self.z_cpu)
            + alphalog_3 * 3.0 * (np.log1p(self.z_cpu) ** 2.0)
        )
        # We put the absolute value for the Jacobian (because this is needed for probabilities)
...
class galaxy_MF(object):
    def __init__(
        self, band=None, Mmin=None, Mmax=None, Mstar=None, alpha=None, phistar=None
    ):
        """
        ...
        if band is None:
            self.Mmin, self.Mmax, self.Mstar, self.alpha, self.phistar = (
                Mmin,
                Mmax,
                Mstar,
                alpha,
    ...
    def build_MF(self, cosmology):
        ...
        self.phistarobs = self.phistar * np.power(cosmology.little_h, 3.0)
        xmax = np.power(10.0, 0.4 * (self.Mstarobs - self.Mminobs))
        xmin = np.power(10.0, 0.4 * (self.Mstarobs - self.Mmaxobs))
        # Check if you need to replace this with a numerical integral.
        self.norm = self.phistarobs * float(
            mpmath.gammainc(self.alpha + 1, a=xmin, b=xmax)
        )
    ...
    def log_evaluate(self, M):
        ...
        toret = (
            xp.log(0.4 * xp.log(10) * self.phistarobs)
            + ((self.alpha + 1) * 0.4 * (self.Mstarobs - M)) * xp.log(10.0)
            - xp.power(10.0, 0.4 * (self.Mstarobs - M))
        )
    ...
    def sample(self, N):
        ...
        return np.interp(
            randomcdf, cdfeval, sarray, left=self.Mminobs, right=self.Mmaxobs
        )
    ...
    def build_effective_number_density_interpolant(self, epsilon):
        ...
        xmin = np.power(10.0, 0.4 * (self.Mstar - maxv))
        for i in range(len(Mvector_interpolant)):
            xmax = np.power(10.0, 0.4 * (self.Mstar - Mvector_interpolant[i]))
            self.effective_density_interpolant[i] = float(
                mpmath.gammainc(self.alpha + 1 + epsilon, a=xmin, b=xmax)
            )

        self.effective_density_interpolant_cpu = self.effective_density_interpolant[
            ::-1
        ]
        self.xvector_interpolant_cpu = self.Mstar - Mvector_interpolant[::-1]

        if is_there_cupy():
            self.effective_density_interpolant_gpu = np2cp(
                self.effective_density_interpolant[::-1]
            )
    ...
    def background_effective_galaxy_density(self, Mthr):
        ...
        outp = self.phistarobs * xp.interp(
            ravelled,
            xvector_interpolant,
            effective_density_interpolant,
            left=effective_density_interpolant[0],
            right=effective_density_interpolant[-1],
        )
...
class kcorr(object):
    ...
    def __call__(self, z):
        ...
        if self.band == "W1":
            k_corr = -1 * (
                4.44e-2 + 2.67 * z + 1.33 * (z**2.0) - 1.59 * (z**3.0)
            )  # From Maciej email
        elif self.band == "K":
            ...
            k_corr = (z + 6 * xp.power(z, 2.0)) / (1 + 15.0 * xp.power(z, 3.0))
        return k_corr
...
class md_rate(basic_redshift_rate):
    ...
    def log_evaluate(self, z):
        xp = get_module_array(z)
        return (
            xp.log1p(xp.power(1 + self.zp, -self.gamma - self.kappa))
            + self.gamma * xp.log1p(z)
            - xp.log1p(xp.power((1 + z) / (1 + self.zp), self.gamma + self.kappa))
        )
