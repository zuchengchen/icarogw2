...
from .cosmology import (
    alphalog_astropycosmology,
    cM_astropycosmology,
    extraD_astropycosmology,
    Xi0_astropycosmology,
    astropycosmology,
)
...
from .priors import (
    ...
    absL_PL_inM,
    conditional_2dimpdf,
    piecewise_constant_2d_distribution_normalized,
)
import copy
...
class Xi0_mod_wrap(object):
    ...
    def update(self, **kwargs):
        ...
        self.cosmology.build_cosmology(
            self.bgwrap.astropycosmo(**bgdict), Xi0=kwargs["Xi0"], n=kwargs["n"]
        )
...
class extraD_mod_wrap(object):
    ...
    def update(self, **kwargs):
        bgdict = {key: kwargs[key] for key in self.bgwrap.population_parameters}
        self.cosmology.build_cosmology(
            self.bgwrap.astropycosmo(**bgdict),
            D=kwargs["D"],
            n=kwargs["n"],
            Rc=kwargs["Rc"],
        )
...
class cM_mod_wrap(object):
    ...
    def update(self, **kwargs):
        bgdict = {key: kwargs[key] for key in self.bgwrap.population_parameters}
        self.cosmology.build_cosmology(
            self.bgwrap.astropycosmo(**bgdict), cM=kwargs["cM"]
        )
...
class alphalog_mod_wrap(object):
    ...
    def update(self, **kwargs):
        ...
        self.cosmology.build_cosmology(
            self.bgwrap.astropycosmo(**bgdict),
            alphalog_1=kwargs["alphalog_1"],
            alphalog_2=kwargs["alphalog_2"],
            alphalog_3=kwargs["alphalog_3"],
        )
...
class massprior_PowerLaw(source_mass_default):
    ...
    def update(self, **kwargs):
        p1, p2 = PowerLaw(kwargs["mmin"], kwargs["mmax"], -kwargs["alpha"]), PowerLaw(
            kwargs["mmin"], kwargs["mmax"], kwargs["beta"]
        )
...
class massprior_PowerLawPeak(source_mass_default):
    ...
    def update(self, **kwargs):
        ...
        p2 = SmoothedProb(
            PowerLaw(kwargs["mmin"], kwargs["mmax"], kwargs["beta"]), kwargs["delta_m"]
        )
        self.prior = conditional_2dimpdf(p1, p2)
    ...
    def update(self, **kwargs):
        p1 = SmoothedProb(
            PowerLawTwoGaussians(
                ...
                kwargs["sigma_g_low"],
                kwargs["mmin"],
                kwargs["mu_g_low"] + 5 * kwargs["sigma_g_low"],
                kwargs["mu_g_high"],
                kwargs["sigma_g_high"],
                kwargs["mmin"],
                kwargs["mu_g_high"] + 5 * kwargs["sigma_g_high"],
            ),
            kwargs["delta_m"],
        )
        p2 = SmoothedProb(
...
class massprior_PowerLaw_NSBH(source_mass_default):
    ...
    def update(self, **kwargs):
        p1, p2 = PowerLaw(kwargs["mmin"], kwargs["mmax"], -kwargs["alpha"]), PowerLaw(
            kwargs["mmin_NS"], kwargs["mmax_NS"], kwargs["beta"]
        )
        self.prior = conditional_2dimpdf(p1, p2)
...
class massprior_PowerLawPeak_NSBH(source_mass_default):
    ...
    def update(self, **kwargs):
        ...
        p2 = SmoothedProb(
            PowerLaw(kwargs["mmin_NS"], kwargs["mmax_NS"], kwargs["beta"]),
            kwargs["delta_m_NS"],
        )
        self.prior = conditional_2dimpdf(p1, p2)
...
class massprior_BinModel2d(source_mass_default):
    ...
    def update(self, **kwargs):
        kwargs_bin_parameters = xp.array(
            [kwargs[key] for key in self.bin_parameter_list]
        )

        pdf_dist = piecewise_constant_2d_distribution_normalized(
            kwargs["mmin"], kwargs["mmax"], kwargs_bin_parameters
        )
...
class spinprior_default(object):
    ...
    def update(self, **kwargs):
        ...
        self.aligned_pdf = TruncatedGaussian(1.0, kwargs["sigma_t"], -1.0, 1.0)
    ...
    def log_pdf(self, chi_1, chi_2, cos_t_1, cos_t_2):
        ...
        return (
            self.beta_pdf.log_pdf(chi_1)
            + self.beta_pdf.log_pdf(chi_2)
            + xp.log(
                self.csi_spin * self.aligned_pdf.pdf(cos_t_1)
                + (1.0 - self.csi_spin) * 0.5
            )
            + xp.log(
                self.csi_spin * self.aligned_pdf.pdf(cos_t_2)
                + (1.0 - self.csi_spin) * 0.5
            )
        )
...
class spinprior_gaussian(object):
    ...
    def update(self, **kwargs):
        self.pdf_evaluator = Bivariate2DGaussian(
            x1min=-1.0,
            x1max=1.0,
            x1mean=kwargs["mu_chi_eff"],
            x2min=0.0,
            x2max=1.0,
            x2mean=kwargs["mu_chi_p"],
            x1variance=kwargs["sigma_chi_eff"] ** 2.0,
            x12covariance=kwargs["rho"]
            * kwargs["sigma_chi_eff"]
            * kwargs["sigma_chi_p"],
            x2variance=kwargs["sigma_chi_p"] ** 2.0,
        )
...
class spinprior_ECOs_totally_reflective(object):
    ...
    def get_chi_crit(self, eps):
        ...
        return xp.pi * (1.0 + self.q) / (2 * xp.abs(xp.log(eps)))
    ...
    def update(self, **kwargs):
        ...
        self.beta_pdf = BetaDistribution(self.alpha_chi, self.beta_chi)
        self.truncatedbeta_pdf = TruncatedBetaDistribution(
            self.alpha_chi, self.beta_chi, self.chi_crit
        )
        self.truncatedgaussian_pdf = TruncatedGaussian(
            self.chi_crit, self.sigma, 0.0, self.chi_crit
        )
        self.lambda_eco = (
            1 - self.beta_pdf.cdf(np.array([self.get_chi_crit(self.eps)]))[0]
        )

    def pdf(self, chi_1, chi_2):
        p_chi_1 = self.f_eco * (
            (1 - self.lambda_eco) * self.truncatedbeta_pdf.pdf(chi_1)
            + self.lambda_eco * self.truncatedgaussian_pdf.pdf(chi_1)
        ) + (1 - self.f_eco) * self.beta_pdf.pdf(chi_1)
        p_chi_2 = self.f_eco * (
            (1 - self.lambda_eco) * self.truncatedbeta_pdf.pdf(chi_2)
            + self.lambda_eco * self.truncatedgaussian_pdf.pdf(chi_2)
        ) + (1 - self.f_eco) * self.beta_pdf.pdf(chi_2)
