...
class hierarchical_likelihood(bilby.Likelihood):
    def __init__(
        ...
        super().__init__(
            parameters={ll: None for ll in self.rate_model.population_parameters}
        )
    ...
    def log_likelihood(self):
        ...
        self.rate_model.update(
            **{
                key: self.parameters[key]
                for key in self.rate_model.population_parameters
            }
        )
        ...
        if (Neff < self.neffINJ) | (Neff == 0.0):
            return float(xp.nan_to_num(-xp.inf))

        # Update the weights on the PE
        self.posterior_samples_dict.update_weights(self.rate_model)
        if xp.any(
            self.posterior_samples_dict.get_effective_number_of_PE() < self.neffPE
        ):
            return float(xp.nan_to_num(-xp.inf))

        integ = (
            self.posterior_samples_dict.log_weights
        )  # Extract a matrix N_ev X N_samples of log weights

        # Combine all the terms
        if self.rate_model.scale_free:
            # Log likelihood for scale free model, Eq. 1.3 on the document
            log_likeli = xp.sum(
                xp.log(self.posterior_samples_dict.sum_weights)
            ) - self.posterior_samples_dict.n_ev * xp.log(self.injections.pseudo_rate)
        else:
            Nexp = self.injections.expected_number_detections()
            # Log likelihood for  the model, Eq. 1.1 on the document
            log_likeli = (
                -Nexp
                + self.posterior_samples_dict.n_ev * xp.log(self.injections.Tobs)
                + xp.sum(xp.log(self.posterior_samples_dict.sum_weights))
            )
