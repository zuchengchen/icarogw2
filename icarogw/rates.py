...
class CBC_catalog_vanilla_rate_skymap(object):
    ...
    def __init__(
        self, catalog, cosmology_wrapper, rate_wrapper, average=False, scale_free=False
    ):
        """
        ...
        if scale_free:
            self.population_parameters = (
                self.cw.population_parameters + self.rw.population_parameters
            )
        else:
            self.population_parameters = (
                self.cw.population_parameters + self.rw.population_parameters + ["Rgal"]
            )
    ...
    def log_rate_PE(self, prior, **kwargs):
        ...
        dNgal_cat, dNgal_bg = self.catalog.effective_galaxy_number_interpolant(
            z,
            kwargs["sky_indices"],
            self.cw.cosmology,
        ...
        log_weights = (
            self.rw.rate.log_evaluate(z)
            + xp.log(dNgaleff)
            - xp.log1p(z)
            - xp.log(xp.abs(self.cw.cosmology.ddl_by_dz_at_z(z)))
            - xp.log(prior)
        )
...
class CBC_low_latency_skymap_EM_counterpart(object):
    def __init__(
        self, cosmology_wrapper, rate_wrapper, list_of_skymaps, scale_free=False
    ):
        """
        ...
        if scale_free:
            ...
            self.population_parameters = (
                self.cw.population_parameters + self.rw.population_parameters + ["R0"]
            )
    ...
    def log_rate_PE(self, prior, **kwargs):
        ...
        log_dVc_dz = xp.log(
            self.cw.cosmology.dVc_by_dzdOmega_at_z(kwargs["z_EM"]) * 4 * xp.pi
        )

        # Sum over posterior samples in Eq. 1.1 on the icarogw2.0 document
        log_weights = (
            self.rw.rate.log_evaluate(kwargs["z_EM"])
            + log_dVc_dz
            - xp.log(prior)
            - xp.log1p(kwargs["z_EM"])
        )

        n_ev = kwargs["z_EM"].shape[0]
        lwtot = xp.empty(kwargs["z_EM"].shape)
        for i in range(n_ev):
            self.list_of_skymaps[i].intersect_EM_PE(
                kwargs["right_ascension"][i, :], kwargs["declination"][i, :]
            )
            log_l_skymap = xp.log(
                self.list_of_skymaps[i].evaluate_3D_likelihood_intersected(
                    dl_samples[i, :]
                )
            )
            lwtot[i, :] = sx.special.logsumexp(
                log_weights[i, :] + log_l_skymap
            ) - xp.log(kwargs["z_EM"].shape[1])
...
class CBC_vanilla_EM_counterpart(object):
    ...
    def __init__(
        ...
        if scale_free:
            self.population_parameters = (
                self.cw.population_parameters
                + self.mw.population_parameters
                + self.rw.population_parameters
            )
        else:
            self.population_parameters = (
                self.cw.population_parameters
        ...
        if self.sw is not None:
            self.population_parameters = (
                self.population_parameters + self.sw.population_parameters
            )
    ...
    def update(self, **kwargs):
        ...
        if self.sw is not None:
            self.sw.update(
                **{key: kwargs[key] for key in self.sw.population_parameters}
            )
    ...
    def log_rate_PE(self, prior, **kwargs):
        ...
        ms1, ms2, z = detector2source(
            kwargs["mass_1"],
            kwargs["mass_2"],
            kwargs["luminosity_distance"],
            self.cw.cosmology,
        )
        log_dVc_dz = xp.log(self.cw.cosmology.dVc_by_dzdOmega_at_z(z) * 4 * xp.pi)
        ...
        # Sum over posterior samples in Eq. 1.1 on the icarogw2.0 document
        log_weights = (
            self.mw.log_pdf(ms1, ms2)
            + self.rw.rate.log_evaluate(z)
            + log_dVc_dz
            - xp.log(prior)
            - xp.log(detector2source_jacobian(z, self.cw.cosmology))
            - xp.log1p(z)
        )

        if self.sw is not None:
            log_weights += self.sw.log_pdf(
                **{key: kwargs[key] for key in self.sw.event_parameters}
            )
        ...
        for i in range(n_ev):
            ...
            lwtot[i, :] = (
                sx.special.logsumexp(log_weights[i, :])
                - xp.log(kwargs["mass_1"].shape[1])
                + np2cp(kde_fit.logpdf(cp2np(kwargs["z_EM"][i, :])))
            )
...
class CBC_catalog_vanilla_rate(object):
    ...
    def log_rate_PE(self, prior, **kwargs):
        ...
        if self.sw is not None:
            log_weights += self.spin_wrap.log_pdf(
                **{
                    key: self.posterior_parallel[key]
                    for key in self.sw.event_parameters
                }
            )
