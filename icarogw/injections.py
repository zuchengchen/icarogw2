...
class injections(object):
    def __init__(self, injections_dict, prior, ntotal, Tobs):
        ...
        self.injections_data_original = {
            key: injections_dict[key] for key in injections_dict.keys()
        }
        self.injections_data = {
            key: injections_dict[key] for key in injections_dict.keys()
        }
    ...
    def update_cut(self, detection_index):
        ...
        self.injections_data = {
            key: self.injections_data_original[key][detection_index]
            for key in self.injections_data_original.keys()
        }
    ...
    def cupyfy(self):
        ...
        self.injections_data_original = {
            key: np2cp(self.injections_data_original[key])
            for key in self.injections_data_original.keys()
        }
        self.injections_data = {
            key: np2cp(self.injections_data[key])
            for key in self.injections_data_original.keys()
        }
    ...
    def numpyfy(self):
        ...
        self.injections_data_original = {
            key: cp2np(self.injections_data_original[key])
            for key in self.injections_data_original.keys()
        }
        self.injections_data = {
            key: cp2np(self.injections_data[key])
            for key in self.injections_data_original.keys()
        }
    ...
    def effective_injections_number(self):
        ...
        var = (
            xp.exp(sx.special.logsumexp(2 * self.log_weights)) / (self.ntotal**2)
            - (mean**2) / self.ntotal
        )
        return (mean**2) / var
    ...
    def pixelize(self, nside):
        ...
        self.injections_data["sky_indices"] = radec2indeces(
            ...
            cp2np(self.injections_data["declination"]),
            nside,
        )
    ...
    def update_weights(self, rate_wrapper):
        ...
        self.log_weights = rate_wrapper.log_rate_injections(
            self.prior,
            **{
                key: self.injections_data[key]
                for key in rate_wrapper.injections_parameters
            }
        )
        xp = get_module_array(self.log_weights)
        sx = get_module_array_scipy(self.log_weights)
        self.pseudo_rate = (
            xp.exp(sx.special.logsumexp(self.log_weights)) / self.ntotal
        )  # Eq. 1.5 on the overleaf documentation
    ...
    def return_reweighted_injections(self, Nsamp, replace=True):
        ...
        return {
            key: self.injections_data[key][idx]
            for key in list(self.injections_data.keys())
        }
