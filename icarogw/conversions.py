...
def chirp_mass(m1, m2):
    ...
    return xp.power(m1 * m2, 3.0 / 5) / xp.power(m1 + m2, 1.0 / 5)
...
class ligo_skymap(object):
    ...
    def intersect_EM_PE(self, ra, dec):
        ...
        if not self.intersected:
            ...
            px_area = ah.nside_to_pixel_area(
                ah.level_to_nside(level)
            ).value  # Pixel area in steradians
            nside = ah.level_to_nside(level)

            match_ipix = xp.zeros(len(ra), dtype=int)
            for i in range(len(ra)):
                mm = ah.lonlat_to_healpix(
                    ra[i], dec[i], nside, order="nested"
                )  # Pixels corresponding to provided ra and dec
    ...
    def evaluate_3D_posterior_intersected(self, dl):
        ...
        """
        xp = np
        pdl_radec = xp.power(2 * xp.pi * (self.dl_sigmas**2.0), -2.0) * xp.exp(
            -0.5 * xp.power((dl - self.dl_means) / self.dl_sigmas, 2.0)
        )
    ...
    def evaluate_3D_likelihood_intersected(self, dl):
        ...
        return (
            self.evaluate_3D_posterior_intersected(dl)
            * self.pixels_area
            / xp.power(dl, 2.0)
        )
    ...
    def evaluate_3D_posterior_likelihood(self, dl, ra, dec):
        ...
        pdl_radec = xp.power(2 * xp.pi * (dl_sigmas**2.0), -2.0) * xp.exp(
            -0.5 * xp.power((dl - dl_means) / dl_sigmas, 2.0)
        )
        prob = sky_prob_rad2 * pdl_radec

        return prob, prob * pixels_area / xp.power(dl, 2.0)
    ...
    def sample_3d_space(self, Nsamp):
        ...
        for i, pix in enumerate(match_ipix):
            ...
            if dl_mean < 0:
                print(
                    "Skipping iteration {:d}, pixel {:d}, negative dl_mean {:.2f} Mpc".format(
                        i, pix, dl_mean
                    )
                )
                continue
...
def cartestianspins2chis(s1x, s1y, s1z, s2x, s2y, s2z, q):
    ...
    chi_1 = xp.sqrt(s1x**2.0 + s1y**2.0 + s1z**2.0)
    chi_2 = xp.sqrt(s2x**2.0 + s2y**2.0 + s2z**2.0)
...
def chi_effective_prior_from_aligned_spins(q, aMax, xs):
    ...
    pdfs[caseA] = (1.0 + q[caseA]) ** 2.0 * (aMax - x_A) / (4.0 * q[caseA] * aMax**2)
    pdfs[caseB] = (1.0 + q[caseB]) ** 2.0 * (aMax + x_B) / (4.0 * q[caseB] * aMax**2)
    pdfs[caseC] = (1.0 + q[caseC]) / (2.0 * aMax)
...
def chi_effective_prior_from_isotropic_spins(q, aMax, xs):
    ...
    # Set up various piecewise cases
    pdfs = xp.ones(xs.size, dtype=complex) * (-1.0)
    caseZ = xs == 0
    ...
    pdfs[caseZ] = (1.0 + q_Z) / (2.0 * aMax) * (2.0 - xp.log(q_Z))
    ...
    pdfs[caseA] = (
        ...
        * (
            ...
            * (
                4.0
                + 2.0 * xp.log(aMax)
                - xp.log(q_A**2 * aMax**2 - (1.0 + q_A) ** 2 * x_A**2)
            )
            - 2.0 * (1.0 + q_A) * x_A * xp.arctanh((1.0 + q_A) * x_A / (q_A * aMax))
            + (1.0 + q_A)
            * x_A
            * (
                Di(-q_A * aMax / ((1.0 + q_A) * x_A))
                - Di(q_A * aMax / ((1.0 + q_A) * x_A))
            )
        )
    )
    ...
    pdfs[caseB] = (
        ...
        * (
            4.0 * q_B * aMax
            + 2.0 * q_B * aMax * xp.log(aMax)
            - 2.0 * (1.0 + q_B) * x_B * xp.arctanh(q_B * aMax / ((1.0 + q_B) * x_B))
            - q_B * aMax * xp.log((1.0 + q_B) ** 2 * x_B**2 - q_B**2 * aMax**2)
            + (1.0 + q_B)
            * x_B
            * (
                Di(-q_B * aMax / ((1.0 + q_B) * x_B))
                - Di(q_B * aMax / ((1.0 + q_B) * x_B))
            )
        )
    )
    ...
    pdfs[caseC] = (
        ...
        / (4.0 * q_C * aMax**2)
        * (
            2.0 * (1.0 + q_C) * (aMax - x_C)
            - (1.0 + q_C) * x_C * xp.log(aMax) ** 2.0
            + (aMax + (1.0 + q_C) * x_C * xp.log((1.0 + q_C) * x_C))
            * xp.log(q_C * aMax / (aMax - (1.0 + q_C) * x_C))
            - (1.0 + q_C)
            * x_C
            * xp.log(aMax)
            * (2.0 + xp.log(q_C) - xp.log(aMax - (1.0 + q_C) * x_C))
            + q_C * aMax * xp.log(aMax / (q_C * aMax - (1.0 + q_C) * x_C))
            + (1.0 + q_C)
            * x_C
            * xp.log(
                (aMax - (1.0 + q_C) * x_C) * (q_C * aMax - (1.0 + q_C) * x_C) / q_C
            )
            + (1.0 + q_C)
            * x_C
            * (
                Di(1.0 - aMax / ((1.0 + q_C) * x_C))
                - Di(q_C * aMax / ((1.0 + q_C) * x_C))
            )
        )
    )
    ...
    pdfs[caseD] = (
        ...
        * (
            -x_D * xp.log(aMax) ** 2
            + 2.0 * (1.0 + q_D) * (aMax - x_D)
            + q_D * aMax * xp.log(aMax / ((1.0 + q_D) * x_D - q_D * aMax))
            + aMax * xp.log(q_D * aMax / (aMax - (1.0 + q_D) * x_D))
            - x_D
            * xp.log(aMax)
            * (
                2.0 * (1.0 + q_D)
                - xp.log((1.0 + q_D) * x_D)
                - q_D * xp.log((1.0 + q_D) * x_D / aMax)
            )
            + (1.0 + q_D)
            * x_D
            * xp.log(
                (-q_D * aMax + (1.0 + q_D) * x_D) * (aMax - (1.0 + q_D) * x_D) / q_D
            )
            + (1.0 + q_D)
            * x_D
            * xp.log(aMax / ((1.0 + q_D) * x_D))
            * xp.log((aMax - (1.0 + q_D) * x_D) / q_D)
            + (1.0 + q_D)
            * x_D
            * (
                Di(1.0 - aMax / ((1.0 + q_D) * x_D))
                - Di(q_D * aMax / ((1.0 + q_D) * x_D))
            )
        )
    )
    ...
    pdfs[caseE] = (
        ...
        / (4.0 * q_E * aMax**2)
        * (
            2.0 * (1.0 + q_E) * (aMax - x_E)
            - (1.0 + q_E) * x_E * xp.log(aMax) ** 2
            + xp.log(aMax)
            * (
                aMax
                - 2.0 * (1.0 + q_E) * x_E
                - (1.0 + q_E) * x_E * xp.log(q_E / ((1.0 + q_E) * x_E - aMax))
            )
            - aMax * xp.log(((1.0 + q_E) * x_E - aMax) / q_E)
            + (1.0 + q_E)
            * x_E
            * xp.log(
                ((1.0 + q_E) * x_E - aMax) * ((1.0 + q_E) * x_E - q_E * aMax) / q_E
            )
            + (1.0 + q_E)
            * x_E
            * xp.log((1.0 + q_E) * x_E)
            * xp.log(q_E * aMax / ((1.0 + q_E) * x_E - aMax))
            - q_E * aMax * xp.log(((1.0 + q_E) * x_E - q_E * aMax) / aMax)
            + (1.0 + q_E)
            * x_E
            * (
                Di(1.0 - aMax / ((1.0 + q_E) * x_E))
                - Di(q_E * aMax / ((1.0 + q_E) * x_E))
            )
        )
    )
    ...
    if xp.any(pdfs == -1):
        ...
        pdfs[boundary] = 0.5 * (
            chi_effective_prior_from_isotropic_spins(
                q[boundary], aMax, xs[boundary] + 1e-6
            )
            + chi_effective_prior_from_isotropic_spins(
                q[boundary], aMax, xs[boundary] - 1e-6
            )
...
def chi_p_prior_from_isotropic_spins(q, aMax, xs):
    ...
    pdfs[caseA] = (
        (1.0 / (aMax**2 * q_A))
        * ((4.0 + 3.0 * q_A) / (3.0 + 4.0 * q_A))
        * (
            xp.arccos((4.0 + 3.0 * q_A) * x_A / ((3.0 + 4.0 * q_A) * q * aMax))
            * (aMax - xp.sqrt(aMax**2 - x_A**2) + x_A * xp.arccos(x_A / aMax))
            + xp.arccos(x_A / aMax)
            * (
                aMax * q_A * (3.0 + 4.0 * q_A) / (4.0 + 3.0 * q_A)
                - xp.sqrt(
                    aMax**2 * q_A**2 * ((3.0 + 4.0 * q_A) / (4.0 + 3.0 * q_A)) ** 2
                    - x_A**2
                )
                + x_A
                * xp.arccos((4.0 + 3.0 * q_A) * x_A / ((3.0 + 4.0 * q_A) * aMax * q_A))
            )
        )
    )

    pdfs[caseB] = (1.0 / aMax) * xp.arccos(x_B / aMax)

    return pdfs.reshape(origin_shape)


# LVK Reviewed
def joint_prior_from_isotropic_spins(
    q, aMax, xeffs, xps, ndraws=10000, bw_method="scott"
):
    """
    Function from https://github.com/tcallister/effective-spin-priors. Credit: T. Callister
    Derivation of equations can be found in arxiv:2104.09508
    ...
    for i in tqdm(range(len(p_chi_eff)), desc="Calculating p(chi_p|chi_eff,q)"):
        p_chi_p_given_chi_eff[i] = chi_p_prior_given_chi_eff_q(
            q[i], aMax, xeffs[i], xps[i], ndraws, bw_method
        )
...
def chi_p_prior_given_chi_eff_q(q, aMax, xeff, xp, ndraws=10000, bw_method="scott"):
    ...
    cost2 = 2.0 * xp.random.random(ndraws) - 1.0
    ...
    while xp.any(cost1 < -1) or xp.any(cost1 > 1):
        ...
        cost2[to_replace] = 2.0 * xp.random.random(to_replace.size) - 1.0
        cost1 = (xeff * (1.0 + q) - q * a2 * cost2) / a1
    ...
    if (1.0 + q) * xp.abs(xeff) / q < aMax:
        max_Xp = aMax
    else:
        max_Xp = xp.sqrt(aMax**2 - ((1.0 + q) * xp.abs(xeff) - q) ** 2.0)
...
def chi_eff_from_spins(chi1, chi2, cos1, cos2, q):
    ...
    to_ret = (chi1 * cos1 + q * chi2 * cos2) / (1.0 + q)
    return to_ret
...
def chi_p_from_spins(chi1, chi2, cos1, cos2, q):
    ...
    sin1 = xp.sqrt(1.0 - cos1**2)
    sin2 = xp.sqrt(1.0 - cos2**2)
...
def radec2skymap(ra, dec, nside):
    ...
    dOmega_deg2 = xp.power(180 / xp.pi, 2.0) * dOmega_sterad
...
def M2L(M):
    ...
    return 3.0128e28 * xp.power(10.0, -0.4 * M)
...
def M2m(M, dl, kcorr):
    ...
    dist_modulus = 5 * xp.log10(dl) + 25.0
...
def detector2source_jacobian(z, cosmology):
    ...
    return xp.abs(xp.power(1 + z, 2.0) * cosmology.ddl_by_dz_at_z(z))
...
def source2detector_jacobian(z, cosmology):
    ...
    cosmo:  class from the cosmology module
        Cosmology class from the cosmology module
    """
    return 1.0 / detector2source_jacobian(z, cosmology)
